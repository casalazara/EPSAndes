...
class SQLCita 
{
	...
	public List<Object[]> reqC11 (PersistenceManager pm){
		String sql="with tt1 as(SELECT (COUNT(s.TIPO))                         ca, " + 
				"                  s.TIPO as tipo, " + 
				...
				"    FROM  t1, " + 
				"         (SELECT MAX(ca) C1, SEMANA " + 
				"          FROM (SELECT ca,  NOMBRE, SEMANA FROM t1) " + 
				"          GROUP BY SEMANA) t2 " + 
				...
				"    FROM tu1, " + 
				"         (SELECT MAX(ca) C1, SEMANA " + 
				"          FROM (SELECT ca,id_afiliado,SEMANA " + 
				"                FROM tu1) " + 
				"          GROUP BY SEMANA) t2 " + 
				"    WHERE tu1.SEMANA = t2.SEMANA " + 
				"      AND tu1.ca = t2.c1 group by tu1.SEMANA, t2.c1 " + 
				"    order by SEMANA DESC), " + 
				"    tb_cantiCita as(SELECT COUNT(DISTINCT c.id_afiliado) AfiliadosNoCitas, " + 
				"                 to_char(to_date(c.FECHA, 'dd/mm/yyyy hh24:mi:ss'), 'WW/YYYY') AS SEMANA " + 
				"          FROM CITA c " + 
				"            WHERE c.CUMPLIDA = 1  " + 
